...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("hit"):
		...
		# Trồng cây
		if tool in [
			DataTypes.Tools.PlantWheat, DataTypes.Tools.PlantTomato, DataTypes.Tools.PlantCarrot,
			DataTypes.Tools.PlantCorn, DataTypes.Tools.PlantRose, DataTypes.Tools.PlantBroccoli
		]:
			if distance >= 20.0:
...
func get_cell_under_mouse() -> void:
	if not tilled_soil_tilemap_layer: return
...
func is_tilled_soil() -> bool:
	if not tilled_soil_tilemap_layer: return false
...
func is_occupied_by_plant() -> bool:
	...
	if not plants_root: return false
...
func plant_seed() -> void:
	...
	# Tìm PlantsUI node
	var plants_ui_node = get_tree().root.find_child("PlantsUI", true, false)
	if plants_ui_node == null:
		print("Không tìm thấy PlantsUI!")
		return

	# Lấy tên item tương ứng từ slot_to_seed (ví dụ: "wheat_seed")
	var seed_item_name = ""
	match seed_type:
		"Wheat":    seed_item_name = "wheat_seed"
		"Tomato":   seed_item_name = "tomato_seed"
		"Carrot":   seed_item_name = "carrot_seed"
		"Corn":     seed_item_name = "corn_seed"
		"Rose":     seed_item_name = "rose_seed"
		"Broccoli": seed_item_name = "broccoli_seed"
		_:
			print("Không hỗ trợ loại hạt giống: ", seed_type)
			return

	# Kiểm tra số lượng từ InventoryManager
	...
	# Trừ 1 hạt giống
	InventoryManager.remove_collectable(seed_item_name, 1)
	
	# Cập nhật lại UI của PlantsUI
	if plants_ui_node.has_method("update_all_quantities"):
		plants_ui_node.update_all_quantities()
	...
	print("Đã gieo ", seed_type, " tại ô ", cell_position, " (còn lại: ", InventoryManager.get_quantity(seed_item_name), ")")
