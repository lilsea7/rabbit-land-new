...
func _ready() -> void:
	...
	# Ẩn PlantsUI và InventoryUI ban đầu
	if plants_ui:
		plants_ui.visible = false
	if inventory_ui:
	...
	# Kết nối các nút
	tool_plants.pressed.connect(_on_tool_plants_pressed)
	tool_iventory.pressed.connect(_on_tool_iventory_pressed)   # ← Kết nối nút Inventory
...
func _on_tool_iventory_pressed() -> void:
	if inventory_ui:
		if inventory_ui.visible:
			...
			inventory_ui.open_inventory()   # Gọi hàm mở và cập nhật inventory
