...
var exp_label: Label  # Không dùng @onready vì node có thể chưa tồn tại

func _ready() -> void:
	# Tạo hoặc lấy ExpLabel
	if has_node("VBoxContainer/ExpLabel"):
		exp_label = $VBoxContainer/ExpLabel
	else:
		exp_label = Label.new()
