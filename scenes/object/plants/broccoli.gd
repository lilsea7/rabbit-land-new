...
# ================== CONFIG ==================
const HARVEST_DAYS: int = 6
const EXP_PLANT: int = 9
const EXP_HARVEST: int = 10

var broccoli_harvest_scene = preload("res://scenes/object/plants/broccoli_harvest.tscn")
...
func on_crop_harvesting() -> void:
	if has_harvested:
		return
	has_harvested = true
	var broccoli_harvest_instance = broccoli_harvest_scene.instantiate() as Node2D
	broccoli_harvest_instance.global_position = global_position
	get_parent().add_child(broccoli_harvest_instance)
	queue_free()
